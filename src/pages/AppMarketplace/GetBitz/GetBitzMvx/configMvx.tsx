...
export const faqs = [
  {
    ...
    content: (
      <>
        ...
        <p className="mt-5">
          You need to use Data NFT and Itheum Core Infrastructure to collect your {`BiTz`} XP, and this exact Web3/Blockchain based product stack can be used by
          you to empower you to take ownership of and tokenize your data. So in essence, you are using Data Ownership + Data Tokenization technology and
  ...
  },
  {
    ...
    content: (
      <>
        ...
        <p className="mt-5">
          A series of new features related to "liveliness & reputation signalling" will soon be introduced within the Itheum protocol, and {`BiTz`} XP stands as
          the inaugural feature of its kind.
        </p>
      </>
    ),
  },
  {
    ...
    content: (
      <>
        {" "}
        <p>
          You need to hold a {`BiTz`} compatible Data NFT in your wallet to play the Get {`BiTz`} game (you are on this page now). This Data NFT was airdropped
          in waves to OGs of the Itheum Protocol, but fear not, you can also get it on the{" "}
          ...
          or any NFT Marketplace (if the OGs broke our hearts and parted ways with their Data NFTs). If this "Gen 1" {`BiTz`} Data NFT is successful, there may
          be a follow-up Series of {`BiTz`} Data NFTs launched and airdropped as well.
        </p>
        <p className="mt-5">
        ...
        <p className="mt-5">
          But in the near future, the Get {`BiTz`} game won't be the only way to collect {`BiTz`} points, if you stay "active" on the Itheum Protocol, you will
          be rewarded with bonus {`BiTz`} points as well. For example, if you use the{" "}
    ...
    content: (
      <>
        <p>
          ...
          there will be LEADERBOARD-based rewards that are tied to use cases within the Itheum protocol. At launch, the following utility will be available:
        </p>
  ...
  },
  {
    ...
    content: (
      <>
        {" "}
        <p>
          We are not game developers and don't pretend to be, so are waiting for an A.I tool that will build the game for us. We'd love for the Get {`BiTz`} app
          to become a hub of "Mini-Games" where you win {`BiTz`} XP. Are you an A.I or a Game Dev and want to build a game layer for the Itheum {`BiTz`} XP
          system? Reach out and you could get a grant from via the{" "}
          <a
