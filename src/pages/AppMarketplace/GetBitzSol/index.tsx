...
import { BlobDataType, computeRemainingCooldown, getApiWeb2Apps, scrollToSection, shortenAddress, sleep } from "./utils";
...
const GetBitzSolView = () => {
  ...
  async function fetchAndLoadLeaderBoards() {
    ...
    try {
      ...
      const res = await fetch(`${getApiWeb2Apps(IS_DEVNET ? "devnet" : "mainnet")}/datadexapi/xpGamePrivate/leaderBoard`, callConfig);
    ...
    try {
      ...
      const res = await fetch(
        `${getApiWeb2Apps(IS_DEVNET ? "devnet" : "mainnet")}/datadexapi/xpGamePrivate/monthLeaderBoard?MMYYString=${MMYYString}`,
        callConfig
      );
      const data = (await res.json()) as LeaderBoardItemType[];
      // const toJSONString = JSON.stringify(data);
  ...
  async function fetchAndLoadMyRankOnLeaderBoard() {
    ...
    try {
      ...
      const res = await fetch(
        `${getApiWeb2Apps(IS_DEVNET ? "devnet" : "mainnet")}/datadexapi/xpGamePrivate/playerRankOnLeaderBoard?playerAddr=${address}`,
        callConfig
      );
